...
class SeparacaoMaterialCreator:
    def __init__(self):
        ...
        # IDs para o subformulário de materiais (baseado na estrutura real)
        self.question_id_subform_materiais = "68891f382a25cf249ab79dd2"  # ID do subformulário na seção Detalhes
        self.sub_question_mapping_materiais = {
            'imagem_material': 'cf08bcae0f9d443f994b9558e73825bd',  # Imagem do Material Separado
            'material': '2fcf125b16c8483c97e4bc8c57ee453b',  # Material (descrição)
            'quantidade': 'fda6238d6c1d4bc79586bd7604d76ccf',  # Quantidade
            'status_produto': 'ec608e7eac574de788e8a6f3794b3bb7',  # Status do Produto
            'separado': 'c6930a0e081242a6a3366f4aa102b106'  # Separado (checkbox)
        }
    ...
    def popular_materiais_no_formulario(self, form_id: str, materiais: List[Dict]):
        ...
        for material in materiais:
            ...
            # Material (descrição) - usa o código_material do cadastro
            if 'codigo_material' in material:
                sub_checklist_questions.append({
                    "question_id": self.sub_question_mapping_materiais['material'], 
                    "value": str(material.get('codigo_material', ""))
                })
            
            # Quantidade - se não vier especificada, usa 1 como padrão
            ...
            # Status do Produto - por padrão "Item a Comprar"
            status = material.get('status_produto', 'Item a Comprar')
            sub_checklist_questions.append({
                "question_id": self.sub_question_mapping_materiais['status_produto'], 
                "value": status
            })
            
            # Separado - campo vazio por padrão (será preenchido manualmente)
            if self.sub_question_mapping_materiais.get('separado'):
                sub_checklist_questions.append({
                    "question_id": self.sub_question_mapping_materiais['separado'], 
                    "value": ""
                })
            
            # Imagem - null por padrão
            if self.sub_question_mapping_materiais.get('imagem_material'):
                sub_checklist_questions.append({
                    "question_id": self.sub_question_mapping_materiais['imagem_material'], 
                    "value": None
                })
